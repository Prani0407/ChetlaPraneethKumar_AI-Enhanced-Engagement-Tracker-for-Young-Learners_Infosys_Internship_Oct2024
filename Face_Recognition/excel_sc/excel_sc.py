...
if not os.path.exists("Praneeth_screenshots(4)"):
    os.makedirs("Praneeth_screenshots(4)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"Praneeth_screenshots(4)/Praneeth_Kumar_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for Praneeth Kumar with screenshot
                    ...
                    screenshot_filename = f"Praneeth Kumar_screenshots(4)/Praneeth_Kumar_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                ...
                # Draw a rectangle around the face and label it as Praneeth Kumar
                for face_location in face_locations:
                    top, right, bottom, left = face_location
                    cv.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
                    cv.putText(frame, 'Praneeth Kumar', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Praneeth Kumar'
            cv.putText(frame, 'Not Praneeth Kumar', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('praneeth_attendance_with_screenshots(4).xlsx', index=False)
            print("DataFrame saved to 'praneeth_attendance_with_screenshots(4).xlsx'")
    ...
    if not df.empty:
        df.to_excel('praneeth_attendance_with_screenshots(4).xlsx', index=False)
        print("Final attendance and screenshots saved to 'praneeth_attendance_with_screenshots(4).xlsx'.")
